...
class LeftSwitchMiddle2Cube(CommandGroup):
    def __init__(self):
        ...
        self.addSequential(DriveStraightCombined(distance=125/12.0, angle=-60, timeout=3))
        self.addSequential(TurnAngle(0, timeout=1))
        self.addSequential(DriveStraightCombined(distance=36/12.0, angle=0, timeout=1))
        self.addSequential(SwitchShoot(timeout=1))
        self.addSequential(DriveStraightCombined(distance=-30/12, angle=0, timeout=1))
        self.addSequential(TurnAngle(45, timeout=1))
        self.addParallel(IntakePosition(timeout=1))
        self.addSequential(DriveStraightCombined(distance=30/12, angle=45, timeout=1))
        self.addParallel(IntakePosition(timeout=1))
        self.addSequential(DriveStraightCombined(distance=-30/12, angle=45, timeout=1))
        self.addParallel(SwitchPosition(timeout=1))
        self.addSequential(TurnAngle(0, timeout=1))
...
class RightSwitchMiddle2Cube(CommandGroup):
    def __init__(self):
        ...
        self.addSequential(setFixedWrist(0.8, timeout= 1))
        self.addSequential(SwitchPosition(timeout=1))
        self.addSequential(DriveStraightCombined(distance=112/12.0, angle=0, timeout=3))
