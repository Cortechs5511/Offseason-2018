...
class MyRobot(wpilib.SampleRobot):
    ...
    def robotInit(self):
        ...
        self.myRobot = wpilib.RobotDrive(frontLeftChannel, rearLeftChannel, frontRightChannel, rearRightChannel)
        self.myRobot.setExpiration(0.1)
        self.stickLeft = wpilib.Joystick(0)
        self.stickRight = wpilib.Joystick(1)
    ...
    def operatorControl(self):
        ...
        while self.isOperatorControl() and self.isEnabled():
            ...
            if self.stickLeft.getRawButton(2):
                self.turnController.setSetpoint(0.0)
                rotateToAngle = True
            elif self.stickLeft.getRawButton(3):
                self.turnController.setSetpoint(90.0)
                rotateToAngle = True
            elif self.stickLeft.getRawButton(4):
                self.turnController.setSetpoint(179.9)
                rotateToAngle = True
            elif self.stickLeft.getRawButton(5):
                self.turnController.setSetpoint(-90.0)
                rotateToAngle = True
            ...
            self.myRobot.tankDrive(self.stickLeft.getY()+currentRotationRate, self.stickRight.getY()+currentRotationRate)
