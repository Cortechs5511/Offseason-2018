...
class DataLogger:
        ...
        if key == '/FMSInfo/FMSControlData':
            ...
            if not self.discard_data:
                ...
                if dlen and dlen % 100 == 0:
                    logger.info("Received %d datapoints (last commanded speed: %.2f)",
                                dlen, value[AUTOSPEED_COL])
    ...
    def wait_for_stationary(self):
        ...
        while True:
            ...
            if abs(last_l_encoder - l_encoder) > 0.01 or \
               abs(last_r_encoder - r_encoder) > 0.01:
    ...
    def run(self):
        ...
        for i, (name, initial_speed, ramp) in enumerate(autonomous):
            ...
            print()
            print("WARNING: It will not automatically stop moving, so disable the robot")
